...
class _ChatTeacherListState extends State<ChatTeacherList> {
  final MainDataGetProvider _mainDataGetProvider = Get.put(
      MainDataGetProvider());
  ...
  final String _classesId = Get
      .put(MainDataGetProvider())
      .mainData['account']['account_division_current']['_id'];
  late var _studyYear;

  search(){
    page = 0;
    EasyLoading.show(status: "جار جلب البيانات");
    chatTeacherProvider.getStudent(page, _classesId, _studyYear, searchController.text);
  }
  ...
  _student(Map _data, String _contentUrl) {
    return ListTile(
            ...
            child: Container(
              ...
              child: Text(
                  ...
                  style: TextStyle(fontSize: 12,
                      fontWeight: _data['chats']['data'][0]['chat_message_isRead']
                          ? FontWeight.normal
                          : FontWeight.bold),
                  maxLines: 1,
      ...
      trailing: Column(
        ...
        children: [
          if (_data['chats']['data'].isNotEmpty) _timeText(
              _data['chats']['data'][0]['created_at']),
          if (_data['chats']['countUnRead'] > 0) _messageUnRead(
              _data['chats']['countUnRead']),
        ],
      ),
      onTap: () async {
        searchController.clear();
        Get.put(ChatMessageStudentProvider()).clear();
        Get.put(ChatMessageStudentProvider()).addListChat(
            _data['chats']['data']);
        await Get.to(() => ChatPage(userInfo: _data, contentUrl: _contentUrl));
        Get
            .put(ChatSocketStudentProvider())
            .socket
...
Widget _bottomSheet(Map _data) {
  return Container(
    decoration: const BoxDecoration(color: MyColor.white1,
