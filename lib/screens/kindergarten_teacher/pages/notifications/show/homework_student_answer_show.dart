...
class _HomeworkStudentAnswerShowState extends State<HomeworkStudentAnswerShow> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: myAppBar("اجابة طالب",MyColor.turquoise),
      body: Column(
        children: [
          Expanded(
            child: ListView(
              shrinkWrap: true,
              children: [
                Padding(padding: const EdgeInsets.all(10), child: imageGrid(widget.contentUrl, widget.data['homework_answers_imgs'],MyColor.turquoise)),
  ...
  _show() {
    Get.defaultDialog(
            ...
            children: [
              Row(
                children: [
                  Radio(
                    fillColor:
                        MaterialStateColor.resolveWith((states) => MyColor.red),
                    focusColor:
                        MaterialStateColor.resolveWith((states) => MyColor.red),
                    value: 1,
        ...
        confirm: RoundedLoadingButton(
          ...
          child: const Text(
            "اضافة",
            style: TextStyle(fontSize: 20, color: MyColor.turquoise),
          ),
