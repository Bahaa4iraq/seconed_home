...
class DegreeStudentList extends StatefulWidget {
  ...
  const DegreeStudentList({Key? key, required this.degreeData})
      : super(key: key);
...
//_formCheck.currentState!.validate()
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: myAppBar(widget.degreeData["degree_exam_name"] ?? '',MyColor.turquoise),
                ...
                MaterialButton(
                    ...
                    onPressed: () {
                      if (_formCheck.currentState!.validate()) {
                        List students = [];
                        students.clear();
                        for (Map _item in widget.degreeData["account_degree"]) {
                          String text = _controller[widget
                                  .degreeData["account_degree"]
                                  .indexOf(_item)]
                              .text;
                          students.add({
                            "student_id": _item['account_id'],
                        ...
                        //Logger().e(_data);
                        DegreeTeacherAPI().insertDegrees(_data).then((result) {
                          showFlushbar(
                              context: context,
                              flushbar: Flushbar(
...
Container _content(Map _data, TextEditingController _textEdit, int _maxDegree) {
  if (_data['account_degree'] != null) {
    _textEdit.text = _data['account_degree'].toString();
  }
  return Container(
          ...
          child: TextFormField(
            ...
            validator: (value) {
              if (value!.length > 3) {
                ...
                if (int.parse(value.removeAllWhitespace) > _maxDegree) {
                  return "الرقم كبير";
                } else {
                  return null;
                }
